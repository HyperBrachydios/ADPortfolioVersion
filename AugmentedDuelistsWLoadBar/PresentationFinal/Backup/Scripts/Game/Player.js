...
//@input SceneObject MonsterContainer;
...
script.api.TakeDamage = function (D) {
    ...
    if (Health <= 0) {
        ...
        for (var i = 0; i < MonsterLimit; i++) {
            script.MonsterContainer.getChild(i).enabled = false;
        }
...
script.api.SetMonster = function (M) {
    ...
    if (MonsterLocation == -1) {
        ...
        if (script.MonsterHealth[MonsterLocation] <= 0) {
        ...
        } else if (script.PrevMonsters[MonsterLocation] == script.PrevMonsters[CurrentMonster]) {
